#!/bin/bash
# D1 SetSTAR + D2 target-vol-norm on 244-ticker universe.
set -e
cd /home/apradipta/phd-research
mkdir -p logs/D1D2_244
for F in 1 2; do
  for S in 42 43 44 45 46; do
    OUT="results/investigation/regime_memory/D1D2_244_fold${F}_seed${S}.json"
    LOG="logs/D1D2_244/fold${F}_seed${S}.log"
    if [ -f "$OUT" ]; then
      echo "[$(date +%H:%M:%S)] skip fold=$F seed=$S"
      continue
    fi
    echo "[$(date +%H:%M:%S)] D1+D2 244 fold=$F seed=$S"
