"""PatchTST baseline trainer using the v2 protocol (matches RAG-STAR).
...
RAG-STAR is the model: this script wraps the vendored PatchTST
architecture (Nie, Nguyen, Sinthong, Kalagnanam, ICLR 2023).

Reference code source vendored at ``src/baselines/vendored/patchtst/``
is adapted from https://github.com/yuqinie98/PatchTST (Apache-2.0).
...
PatchTST-specific knobs default to the ICLR 2023 paper's small config
adjusted for our T=20 lookback (patch_len=4, stride=2 -> 9 patches).

Loss: pure cross-sectional MSE on z-scored 5d forward log returns
(``cs_mse_loss``). PatchTST has no auxiliary loss, so this matches
MASTER / StockMixer / DySTAGE exactly.

Run:
    python -m src.baselines.train_patchtst_v2 --fold 1 --seed 42

Output: results/baselines_244/patchtst_v2/fold{F}_seed{S}.json (+ npz).
"""
...
from src.baselines.patchtst_adapter import PatchTSTAdapter, PatchTSTHyperparams
...
@dataclass
class PatchTSTV2Config(V2BaselineConfig):
    """Top-level config bundling the v2 protocol + PatchTST-specific knobs.

    PatchTST hyperparameter rationale:
      - patch_len=4, stride=2 give 9 patches over a 20-day lookback,
        matching the paper's recommendation for short context windows.
      - d_model=128 matches the ICLR 2023 small config.
      - n_heads=8 (rather than the paper's 16) so each head has 16
        attention dims; with d_model=128 and 16 heads each head only
        gets 8 dims which we found to underfit our cross-sectional task.
      - n_layers=3 matches the paper's small config.
      - dropout=0.1 matches the paper.
      - RevIN ON is the paper's strong default and is especially helpful
        for return-style features that vary in scale across tickers.
    """
    output_dir: str = "results/baselines_244/patchtst_v2"
    # PatchTST learning rate override: bumped from the v2 default (1e-4) to
    # 3e-4 because the model was converging to trivial near-zero predictions
    # at the lower rate (gradient too small to escape the cross-sectional
    # zero minimum).
    ...
    # RevIN OFF: per-instance reversible instance normalisation across the
    # lookback strips per-ticker level information that the cross-sectional
    # head needs for ranking.
    revin: bool = False
    affine: bool = True
...
def main() -> None:
    ...
    cfg = PatchTSTV2Config(fold=args.fold, seed=args.seed)
    if args.smoke:
        cfg.epochs = 2
    ...
    print(f"[PatchTST-v2] fold={cfg.fold} seed={cfg.seed} device={device}")

    x_raw, y, tickers, dates = build_panel(cfg)
    T, N, Fdim = x_raw.shape
    print(f"[PatchTST-v2] panel: T={T} N={N} F={Fdim}")
    if N < 50:
    ...
    print(f"[PatchTST-v2] fold {cfg.fold}: "
    ...
    model = PatchTSTAdapter(hp).to(device)
    ...
    for epoch in range(cfg.epochs):
        ...
        print(f"[PatchTST-v2] epoch {epoch}: train_loss={train_loss:.4f} "
        ...
        if improved:
            ...
            if patience >= cfg.early_stop_patience:
                print(f"[PatchTST-v2] early stop epoch {epoch} "
    ...
    print(f"[PatchTST-v2] TEST ic={test_metrics['ic']:+.4f} "
    ...
    print(f"[PatchTST-v2] wrote {out_path}")
