"""Two-stage fold-causal JOINT (recon + contrastive) pretrain -> finetune
for the canonical BANKLESS InVAR (EXPERIMENT-ONLY; never in any paper).

NOT a paper baseline. This trainer is a COPY of
``src.baselines.train_invar_clpretrain_v2`` with EXACTLY ONE change: the
Stage-1 self-supervised objective JOINTLY optimises BOTH the
masked-window reconstruction objective from
``src.baselines.train_invar_pretrain_v2`` AND the regime-contrastive
objective from ``src.baselines.train_invar_clpretrain_v2``:

    total_loss = recon_loss + LAMBDA * contrastive_loss   (LAMBDA = 1.0)

Everything else (the fold-causal corpus restriction
``pretrain_idx = fold_split(cfg, dates)[0]``, the
``_assert_pretrain_causal`` hard leakage guard, the per-day in-corpus
assertion, train-fold standardisation, the regime-fingerprint key built
on TRAIN days only, the Stage-2 finetune that loads the fold encoder
ckpt into the canonical BANKLESS InVAR with a layer-wise LR + two-regime
val + SWA + cs_mse_loss, the JSON-only disk-safe write, the argparse, the
...
LAMBDA is a module constant (``LAMBDA = 1.0``), documented here so the
recon and contrastive terms are weighted equally and the combination is
reproducible.

Design
------
Stage 1 (JOINT recon + contrastive SSL of the per-ticker temporal
encoder):
    A small pretrain wrapper holds the SAME ``PerTickerTemporalEncoder``
    submodules as the canonical model PLUS:
      * a ``Linear(d -> F)`` reconstruction head (reused verbatim from
        the masked-recon trainer ``train_invar_pretrain_v2``), and
      * a SimCLR-style 2-layer MLP projection head
        (``d -> d -> proj_dim``, reused verbatim from the contrastive
        trainer ``train_invar_clpretrain_v2``).

    For each minibatch of training days t (restricted to
    ``pretrain_idx`` ONLY):
      * RECON term (from train_invar_pretrain_v2): for each day's
        active-ticker ``(N, T, F)`` windows, the SAME span-based random
        masking of (day, feature) cells (mask ratio ~0.5) is applied;
        the encoder's PRE-POOL ``(N, T, d)`` sequence is recomputed via
        the SAME ``encode_sequence`` submodule chain and mapped back
        through ``Linear(d -> F)``; MSE is taken on masked cells ONLY,
        gated by the per-ticker tradable/history mask so padded /
        inactive cells never contribute.
      * CONTRASTIVE term (from train_invar_clpretrain_v2): each day's
        active-ticker last-step encodings are masked-mean-pooled,
        projected, L2-normalised -> one day embedding; positives are the
        nearest in-batch days in the standardised 14-d regime-fingerprint
        key space; supervised-contrastive / InfoNCE with cosine
        similarity and temperature ``tau``.
      * The per-batch loss optimised is
        ``total = recon_loss + LAMBDA * contrastive_loss`` over ONE
        optimiser/scheduler covering {temporal encoder + recon head +
        projection head}.
    ...
    with TRAIN-day stats only. ``_assert_pretrain_causal`` enforces the
    subset/disjoint invariant, and every day index used inside Stage-1
    is asserted to lie in ``pretrain_idx``.

    Only the temporal-encoder ``state_dict`` is saved (both heads are
    discarded) to the SAME fold-keyed checkpoint
    ``results/<out>/_ckpt/foldF_encoder.pt`` with the SAME key
    convention as the masked-recon / contrastive trainers, so Stage-2's
    strict load is unchanged.

Stage 2 (finetune the full BANKLESS InVAR per (fold, seed)):
    BYTE-IDENTICAL to
    ``train_invar_clpretrain_v2.run_stage2_finetune`` /
    ``train_invar_pretrain_v2.run_stage2_finetune`` (same v2_runner
    calls, same SWA EMA loop, same early-stop, same JSON schema, same
    strict ckpt load, same layer-wise LR).
...
Run (1-fold smoke):
    # Stage 1: joint-pretrain fold 1 (1 epoch), save ckpt, exit.
    python -m src.baselines.train_invar_jointpretrain_v2 --fold 1 --seed 42 \
        --panel_kind lattice_native --two_regime_val \
        --pretrain_only --pretrain_epochs 1 \
        --output_dir results/invar_jointpretrain
...
# ============================================================================
# STAGE 1: JOINT (masked-recon + regime-contrastive) self-supervised
# pretrain of the per-ticker temporal encoder.
# ============================================================================

# Joint Stage-1 weighting. The total loss is
# ``total = recon_loss + LAMBDA * contrastive_loss``; LAMBDA = 1.0 weights
# the masked-window reconstruction term and the regime-contrastive
# (InfoNCE) term EQUALLY. Documented module constant for reproducibility.
LAMBDA = 1.0

# Contrastive-pretrain hyperparameters (Stage-1 only; reused VERBATIM
# from train_invar_clpretrain_v2). InfoNCE temperature, the SimCLR
# projection-head width, and the fraction of the in-batch days treated
# as positives for each anchor (nearest in standardised
...
# Masked-window reconstruction hyperparameter (Stage-1 only; reused
# VERBATIM from train_invar_pretrain_v2): the (day, feature) cell mask
# ratio for the span-masking pretext.
RECON_MASK_RATIO = 0.5


class TemporalEncoderJointPretrainer(nn.Module):
    """Joint (masked-recon + regime-contrastive) wrapper around the
    ...
    strict load) PLUS two heads reused verbatim from the single-objective
    trainers:

      * ``recon_head = nn.Linear(d_model, n_features)`` (from
        ``train_invar_pretrain_v2.TemporalEncoderPretrainer``): maps the
        encoder's PRE-POOL ``(N, T, d)`` sequence back to feature space
        for the masked-window reconstruction pretext.
      * ``proj_head`` = 2-layer MLP (``d -> d -> proj_dim``) (from
        ``train_invar_clpretrain_v2.TemporalEncoderContrastivePretrainer``):
        produces one projected, L2-normalised day embedding for the
        regime-contrastive pretext.

    ``encode_sequence`` (from the masked-recon trainer) re-runs the
    canonical encoder UP TO the pre-pool ``(N, T, d)`` sequence WITHOUT
    modifying the original class. ``day_embedding`` (from the contrastive
    trainer) produces the day-level embedding. The contrastive loss lives
    outside this module. After pretrain, ``encoder.state_dict()`` is the
    artefact loaded into the finetune model's ``temporal_encoder``
    submodule (strict key match); BOTH heads are discarded.
    """
    ...
    def __init__(
        ...
        # Reconstruction head: VERBATIM from
        # train_invar_pretrain_v2.TemporalEncoderPretrainer.
    ...
    def encode_sequence(self, x_window: Tensor) -> Tensor:
        ...
        VERBATIM from
        ``train_invar_pretrain_v2.TemporalEncoderPretrainer.encode_sequence``.
    ...
    def reconstruct(self, x_masked: Tensor) -> Tensor:
        """Reconstruct ``(N, T, F)`` from the masked input window.

        VERBATIM from
        ``train_invar_pretrain_v2.TemporalEncoderPretrainer.forward``.
        """
    ...
    def day_embedding(self, x_window: Tensor) -> Tensor:
        ...
        VERBATIM from
        ``train_invar_clpretrain_v2.TemporalEncoderContrastivePretrainer.day_embedding``.
...
def _span_mask(
    ...
    VERBATIM from ``train_invar_pretrain_v2._span_mask``.
...
def _supcon_infonce_loss(
    ...
    VERBATIM from ``train_invar_clpretrain_v2._supcon_infonce_loss``.
...
def run_stage1_pretrain(
    ...
    """Fold-causal JOINT (masked-recon + regime-contrastive)
    self-supervised pretrain of the temporal encoder.
    ...
    enforce this. Per batch:
        total = recon_loss + LAMBDA * contrastive_loss.
    """
    set_seeds(cfg.seed)

    # ---- v2_runner data / fold calls: SAME args, SAME order as
    # train_invar_clpretrain_v2.py / train_invar_pretrain_v2.py /
    # train_invar_stx_v2.py. ----
    x_raw, y, tickers, dates = build_panel(cfg)
    T, N, Fdim = x_raw.shape
    print(f"[InVAR-jointpretrain S1] panel: T={T} N={N} F={Fdim}")
    if N < 50:
    ...
    print(f"[InVAR-jointpretrain S1] fold {cfg.fold}: "
    ...
    # panel exactly as train_invar_clpretrain_v2 / episode_keys do, then
    ...
    # ONE optimiser/scheduler over {temporal encoder + recon head +
    # projection head} (model.parameters() spans all three).
    ...
    print(f"[InVAR-jointpretrain S1] {len(valid_days)} usable train days, "
          f"batch={batch_days}, pos/anchor={n_pos}, tau={CL_TEMPERATURE}, "
          f"mask_ratio={RECON_MASK_RATIO}, LAMBDA={LAMBDA}")
    ...
    for epoch in range(pretrain_epochs):
        ...
        for b0 in range(0, len(perm) - batch_days + 1, batch_days):
            ...
            with torch.amp.autocast(
                ...
                # ---- Per-day: BOTH the contrastive day embedding (from
                # train_invar_clpretrain_v2) AND the masked-window
                # reconstruction term (from train_invar_pretrain_v2). ----
                z_list = []
                recon_terms = []
                for _t in batch:
                    t = int(_t)
                    ...
                    # --- CONTRASTIVE day embedding (verbatim CL path). ---
                    z_list.append(model.day_embedding(x_win))

                    # --- MASKED-WINDOW RECONSTRUCTION (verbatim recon
                    # path from train_invar_pretrain_v2): same span mask,
                    # same pre-pool sequence, same Linear(d->F) head, MSE
                    # on masked cells ONLY gated by tradable history. ---
                    hist_mask_np = tradable[
                        t - W + 1: t + 1
                    ][:, active_idx]
                    hist_mask_np = np.transpose(
                        hist_mask_np, (1, 0)
                    )   # (N, T)
                    ...
                    cell_mask_np = _span_mask(
                        n_active, W, Fdim, RECON_MASK_RATIO, rng
                    )
                    ...
                    # Loss only on cells that are BOTH masked AND on a
                    # tradable day for that ticker.
                    loss_cells = cell_mask & hist_mask.unsqueeze(-1)

                    recon = model.reconstruct(x_masked)     # (N, T, F)
                    if loss_cells.any():
                        diff = (recon - x_win)[loss_cells]
                        recon_terms.append((diff ** 2).mean())
                ...
                # ---- JOINT objective: total = recon + LAMBDA*contrastive
                total_loss = recon_loss + LAMBDA * cl_loss
            ...
            cl_losses.append(float(cl_loss.item()))
        dt = time.time() - t0
        mean_loss = float(np.mean(losses)) if losses else float("nan")
        mean_recon = (float(np.mean(recon_losses))
                      if recon_losses else float("nan"))
        mean_cl = (float(np.mean(cl_losses))
                   if cl_losses else float("nan"))
        print(f"[InVAR-jointpretrain S1] epoch {epoch}: "
              f"total={mean_loss:.5f} recon={mean_recon:.5f} "
              f"infonce={mean_cl:.5f} "
              f"({len(losses)} batches, {dt:.1f}s)")

    # ---- Save the temporal-encoder state_dict ONLY (BOTH heads are
    # discarded). SAME key/path convention as the masked-recon /
    # contrastive trainers so Stage-2's strict load works unchanged. ----
    ...
    print(f"[InVAR-jointpretrain S1] saved encoder ckpt -> {ckpt_path}")
...
def _assert_pretrain_causal(
    ...
    print(f"[InVAR-jointpretrain] LEAKAGE-CHECK OK: |pretrain|={len(p)} "
...
# BYTE-IDENTICAL to train_invar_clpretrain_v2.run_stage2_finetune /
# train_invar_pretrain_v2.run_stage2_finetune.
# ============================================================================
...
def run_stage2_finetune(
    ...
    set_seeds(cfg.seed)
    print(f"[InVAR-jointpretrain S2] fold={cfg.fold} seed={cfg.seed} "
          f"device={device}")
    ...
    x_raw, y, tickers, dates = build_panel(cfg)
    T, N, Fdim = x_raw.shape
    print(f"[InVAR-jointpretrain S2] panel: T={T} N={N} F={Fdim}")
    if N < 50:
        raise RuntimeError("Panel too small")
    ...
    train_idx, val_idx, test_idx = fold_split(cfg, dates)
    print(f"[InVAR-jointpretrain S2] fold {cfg.fold}: "
          f"train={len(train_idx)} val={len(val_idx)} "
          f"test={len(test_idx)}")
    ...
    if not macro_path.exists():
        print("[InVAR-jointpretrain S2] macro parquet missing; building...")
        build_macro_duration_features()
    macro = pd.read_parquet(macro_path)
    macro_arr, macro_cols, _ = standardize_macro_duration(
        macro, dates, train_idx,
    )
    print(f"[InVAR-jointpretrain S2] macro features: {len(macro_cols)} dims")
    ...
    if len(gate_indices) != len(MACRO_GATE_COLS):
        ...
        print(f"[InVAR-jointpretrain S2] WARN missing gate cols: {missing}")
    ...
    print(f"[InVAR-jointpretrain S2] macro_gate input: "
    ...
    if not betas_path.exists():
        print("[InVAR-jointpretrain S2] rolling betas parquet missing; "
              "building...")
        build_rolling_betas()
        betas_path = Path(cfg.biotech_rolling_betas_parquet)
    ...
    print(f"[InVAR-jointpretrain S2] duration input dim: "
    ...
    print(f"[InVAR-jointpretrain S2] loaded pretrained temporal encoder "
    ...
    print(f"[InVAR-jointpretrain S2] layer-wise LR: encoder={0.25 * base_lr:.2e} "
    ...
    for epoch in range(cfg.epochs):
        ...
        print(f"[InVAR-jointpretrain S2] epoch {epoch}: "
        ...
        if improved:
            ...
            if patience >= cfg.early_stop_patience:
                print(f"[InVAR-jointpretrain S2] early stop epoch {epoch} "
    ...
    if cfg.use_swa and ema_state is not None:
        ...
        print("[InVAR-jointpretrain S2] SWA: using final EMA state for test")
    ...
    print(f"[InVAR-jointpretrain S2] TEST ic={test_metrics['ic']:+.4f} "
    ...
    payload = {
        "fold": cfg.fold,
        "seed": cfg.seed,
        "model": "InVAR-jointpretrain (v2 protocol)",
        "panel_T": int(T),
        "panel_N": int(N),
        "panel_F": int(Fdim),
    ...
    print(f"[InVAR-jointpretrain S2] wrote {out_path} (no npz; disk-safe)")


def main() -> None:
    """CLI entry point. Two-stage joint-pretrain -> finetune for
    ...
    p.add_argument("--output_dir", type=str,
                   default="results/invar_jointpretrain")
    ...
    print(f"[InVAR-jointpretrain] fold={cfg.fold} seed={cfg.seed} "
    ...
    # Stage 1 joint pretrain (always seeded with cfg.seed; the sbatch
    # passes --seed 42 for the single per-fold pretrain).
    run_stage1_pretrain(cfg, args.pretrain_epochs, device, ckpt_path)
    if args.pretrain_only:
        print("[InVAR-jointpretrain] --pretrain_only: stage 1 done; "
              "exiting.")
        return
