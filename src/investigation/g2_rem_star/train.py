"""G²-REM-STAR training loop.

REM iter 3B graph path + StockMixer-style no-graph path + τ=0.2 gate.
Folds 1 and 2 for iteration; fold 3 only with authorization.
"""
from __future__ import annotations

import argparse
import json
import math
import time
from dataclasses import asdict, dataclass
from pathlib import Path

import numpy as np
import pandas as pd
import torch

from src.investigation.g2_rem_star.model import G2REMConfig, G2REMStar
from src.investigation.regime_memory.catalog import RegimeCatalog, assign_days_to_cluster, build_catalog
from src.investigation.regime_memory.signature import (
    compute_extended_signatures, compute_signatures, forward_fill_signatures,
)
from src.mtgn.data.risk_features import build_risk_features
from src.mtgn.model.utils.patch_construction import build_patches
from src.mtgn.training.losses import cs_mse_loss
from src.mtgn.training.panel_enriched import (
    EnrichedPanelConfig, build_enriched_panel, panel_to_tensors,
)
from src.mtgn.training.train import information_coefficient, rank_ic
from src.mtgn.training.train_baselines import walk_forward_fold


LOG_RET_INDEX = 0


@dataclass
class G2REMTrainConfig:
    start_date: str = "2015-01-01"
    end_date: str = "2022-12-31"
    horizon_days: int = 5
    max_tickers: int | None = 100
    fold: int = 1
    epochs: int = 15
    patience: int = 5
    seed: int = 42
    lr: float = 1e-4
    weight_decay: float = 1e-5
    warmup_steps: int = 500
    lr_schedule: str = "cosine"
    hidden_dim: int = 128
    num_neighbors: int = 8
    temporal_window: int = 20
    num_heads: int = 4
    num_layers: int = 2
    ff_dim: int = 256
    transformer_dropout: float = 0.1
    head_hidden: int = 64
    head_dropout: float = 0.2
    gate_hidden: int = 16
    gate_temperature: float = 0.2
    use_xstock_mlp: bool = True
    xstock_hidden: int = 8
    sparsity_weight: float = 0.01
    K: int = 4
    num_prototypes: int = 16


def train(cfg: G2REMTrainConfig) -> dict:
    torch.manual_seed(cfg.seed); np.random.seed(cfg.seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    panel_cfg = EnrichedPanelConfig(
        start_date=cfg.start_date, end_date=cfg.end_date,
        horizon_days=cfg.horizon_days, max_tickers=cfg.max_tickers,
    )
    panel, tickers, dates = build_enriched_panel(panel_cfg)
    tensors = panel_to_tensors(panel, tickers, dates)
    x = torch.from_numpy(tensors["x"]).to(device)
    y = torch.from_numpy(tensors["y"]).to(device)
    mask_all = torch.from_numpy(tensors["mask"]).to(device)
    T, N, Fdim = x.shape
    print(f"panel: T={T} N={N} F={Fdim}")

    tr, va, te = walk_forward_fold(dates, cfg.fold, cfg.horizon_days)
    print(f"split [fold{cfg.fold}]: train {tr.stop - tr.start}d  val {va.stop - va.start}d  test {te.stop - te.start}d")

    mu = x[tr].reshape(-1, Fdim).mean(dim=0)
    sd = x[tr].reshape(-1, Fdim).std(dim=0).clamp(min=1e-6)
    x = (x - mu) / sd

    # Regime signatures (4-dim base + 3 enriched = 7-dim for gate)
    log_returns_np = tensors["x"][:, :, LOG_RET_INDEX]
    mask_np = tensors["mask"]
    risk_df = build_risk_features(cfg.start_date, cfg.end_date)
    sigs_base = compute_signatures(log_returns_np, mask_np, dates, risk_df)
    sigs_full = compute_extended_signatures(log_returns_np, mask_np, dates, risk_df)
    sigs_base = forward_fill_signatures(sigs_base)
    sigs_full = forward_fill_signatures(sigs_full)

    # Build REM catalog (uses base 4-dim signatures)
    edge_train_end = str(pd.Timestamp(dates[tr.stop - 1]).date())
    catalog: RegimeCatalog = build_catalog(
        signatures=sigs_base, tickers=tickers, dates=dates,
        log_returns=log_returns_np, mask=mask_np,
        train_slice=tr,
        cfg_start_date=cfg.start_date, cfg_train_end=edge_train_end,
        K=cfg.K, num_neighbors=cfg.num_neighbors, kmeans_seed=0,
        single_graph=True,
    )
    day_cluster = assign_days_to_cluster(sigs_base, catalog)  # unused but kept
    topk_by_c = {c: torch.from_numpy(catalog.top_neighbors[c]).long().to(device)
                 for c in range(cfg.K)}

    # Z-score signatures using train-fold stats (separate for base and full)
    def zscore_train(arr):
        train_arr = arr[tr]
        finite = np.all(np.isfinite(train_arr), axis=1)
        mu_ = train_arr[finite].mean(axis=0)
        sd_ = train_arr[finite].std(axis=0).clip(min=1e-6)
        out = (arr - mu_) / sd_
        return np.where(np.isfinite(out), out, 0.0).astype(np.float32)

    sigs_base_z = torch.from_numpy(zscore_train(sigs_base)).to(device)
    sigs_full_z = torch.from_numpy(zscore_train(sigs_full)).to(device)

    mcfg = G2REMConfig(
        feature_dim=Fdim, hidden_dim=cfg.hidden_dim,
        num_neighbors=cfg.num_neighbors, temporal_window=cfg.temporal_window,
        num_heads=cfg.num_heads, num_layers=cfg.num_layers, ff_dim=cfg.ff_dim,
        transformer_dropout=cfg.transformer_dropout,
        head_hidden=cfg.head_hidden, head_dropout=cfg.head_dropout,
        signature_dim=sigs_full.shape[1], signature_dim_rem=sigs_base.shape[1],
        gate_hidden=cfg.gate_hidden, gate_temperature=cfg.gate_temperature,
        use_xstock_mlp=cfg.use_xstock_mlp, xstock_hidden=cfg.xstock_hidden,
        num_stocks=N, K=cfg.K, num_prototypes=cfg.num_prototypes,
        sparsity_weight=cfg.sparsity_weight,
    )
    model = G2REMStar(mcfg).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay)
    train_days = tr.stop - tr.start
    total_steps = max(1, cfg.epochs * train_days)

    def _lr_lambda(step):
        if cfg.warmup_steps > 0 and step < cfg.warmup_steps:
            return max(1e-3, (step + 1) / cfg.warmup_steps)
        progress = (step - cfg.warmup_steps) / max(1, total_steps - cfg.warmup_steps)
        progress = min(max(progress, 0.0), 1.0)
        return 0.5 * (1.0 + math.cos(math.pi * progress))
    scheduler = torch.optim.lr_scheduler.LambdaLR(opt, _lr_lambda)

    W = cfg.temporal_window

    def run_epoch(sl, train_: bool):
        model.train(train_)
        losses, preds, ys, ms, alphas = [], [], [], [], []
        for t in range(sl.start + W, sl.stop):
            m = mask_all[t]
            if m.sum() < 3:
                continue
            active_idx = m.nonzero(as_tuple=True)[0]
            x_win = x[t - W + 1: t + 1]
            mask_win = mask_all[t - W + 1: t + 1]
            top_k_t = topk_by_c[int(day_cluster[t])]
            patches, patch_mask = build_patches(x_win, mask_win, top_k_t, active_idx)
            self_window = x_win[:, active_idx, :].transpose(0, 1)
            out = model.forward_day(patches, patch_mask, self_window,
                                    sigs_full_z[t], m, cluster_id=int(day_cluster[t]))
            y_hat = out["y_hat"]
            l = cs_mse_loss(y_hat, y[t], m)
            # REM prototype sparsity on soft weights
            if train_ and cfg.sparsity_weight > 0 and out.get("proto_weights") is not None:
                l = l + cfg.sparsity_weight * out["proto_weights"].abs().sum()
            if train_:
                opt.zero_grad(); l.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
                opt.step(); scheduler.step()
            losses.append(l.item())
            preds.append(y_hat.detach().cpu().numpy()[None, :])
            ys.append(y[t].detach().cpu().numpy()[None, :])
            ms.append(m.detach().cpu().numpy()[None, :])
            alphas.append(float(out["alpha"].detach().cpu()))
        yhat_arr = np.concatenate(preds) if preds else np.zeros((0, N))
        y_arr   = np.concatenate(ys)    if ys    else np.zeros((0, N))
        m_arr   = np.concatenate(ms)    if ms    else np.zeros((0, N), dtype=bool)
        return (float(np.mean(losses)) if losses else float("nan"),
                yhat_arr, y_arr, m_arr, alphas)

    history = []
    best_val_ic = -float("inf"); best_state = None; best_epoch = -1; stale = 0
    for epoch in range(cfg.epochs):
        t0 = time.time()
        train_loss, _, _, _, train_a = run_epoch(tr, True)
        val_loss, vp, vy, vm, val_a = run_epoch(va, False)
        v_ic = information_coefficient(vp, vy, vm)
        v_r = rank_ic(vp, vy, vm)
        dt = time.time() - t0
        improved = v_ic > best_val_ic
        print(f"[{epoch:02d}] train_loss={train_loss:.4f}  val_loss={val_loss:.4f}  "
              f"val_ic={v_ic:+.4f}  val_rank_ic={v_r:+.4f}  α_train={np.mean(train_a):.3f}  "
              f"α_val={np.mean(val_a):.3f}  ({dt:.1f}s){'  *best*' if improved else ''}")
        history.append(dict(epoch=epoch, train_loss=train_loss, val_loss=val_loss,
                            val_ic=v_ic, val_rank_ic=v_r,
                            alpha_train=float(np.mean(train_a)),
                            alpha_val=float(np.mean(val_a)),
                            time_sec=round(dt, 2)))
        if improved:
            best_val_ic = v_ic
            best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
            best_epoch = epoch; stale = 0
        else:
            stale += 1
            if stale >= cfg.patience:
                print(f"early stop epoch {epoch} best={best_val_ic:+.4f} @ {best_epoch}")
                break

    if best_state is not None:
        model.load_state_dict(best_state)
    _, tp, ty, tm, test_a = run_epoch(te, False)
    test_ic = information_coefficient(tp, ty, tm)
    test_r = rank_ic(tp, ty, tm)
    print(f"\nTEST ic={test_ic:+.4f}  rank_ic={test_r:+.4f}  α_test mean={np.mean(test_a):.3f} std={np.std(test_a):.3f}")

    return dict(
        panel_T=T, panel_N=N, panel_F=Fdim,
        test_ic=test_ic, test_rank_ic=test_r,
        best_val_ic=best_val_ic, best_epoch=best_epoch,
        test_alpha_mean=float(np.mean(test_a)),
        test_alpha_std=float(np.std(test_a)),
        history=history, config=asdict(cfg), fold=cfg.fold,
        _test_preds=tp.astype(np.float32),
        _test_y=ty.astype(np.float32),
        _test_mask=tm.astype(bool),
        _test_alphas=np.array(test_a, dtype=np.float32),
    )


def main():
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--fold", type=int, required=True, choices=[1, 2, 3])
    p.add_argument("--seed", type=int, required=True)
    p.add_argument("--gate-temp", type=float, default=0.2)
    p.add_argument("--output", type=Path, required=True)
    args = p.parse_args()
    cfg = G2REMTrainConfig(fold=args.fold, seed=args.seed, gate_temperature=args.gate_temp)
    result = train(cfg)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    tp = result.pop("_test_preds"); ty = result.pop("_test_y"); tm = result.pop("_test_mask")
    ta = result.pop("_test_alphas")
    args.output.write_text(json.dumps(result, indent=2, default=str))
    np.savez_compressed(args.output.with_suffix(".npz"),
                        preds=tp, y=ty, mask=tm, test_alphas=ta)
    print(f"\nwrote {args.output}")


if __name__ == "__main__":
    main()
