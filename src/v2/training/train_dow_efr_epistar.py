"""DOW-EFR-epiSTAR: Episodic Factor Repricing OVERLAY on locked DOW.

Per the DOW-EFR spec (revised EFR-DGraph fix):

    Base = locked DOW-epiSTAR (Section E macro-duration head + rate
    memory + Section G duration graph). All DOW machinery preserved
    unchanged.

    EFR overlay (per-(day, ticker)):
        theta_t        = sum_tau softmax(cosine(reg_t, reg_tau)/T) *
                         reliability_tau * theta_tau         (top-K=16)
        score_efr      = B_t @ theta_t                       (B_t in R^{N x K})
        score_efr_z    = per-day cs z-score(score_efr) over active rows
        confidence_t   = sigmoid(3*mean_rel + 1*mean_sim + 1*agreement - 1.5)
        age_gate[t,i]  = sigmoid((age_days - age_protect_days) /
                                  age_protect_steepness)
        lambda_efr[t,i] = max_lambda * confidence_t * age_gate[t,i]

        score_total[t, i] = score_dow[t, i] + lambda_efr[t, i] * score_efr_z[i]

    Loss = L_cs_mse(score_total) + 1e-4 * mean(lambda_efr^2)

    No learnable factor premia. theta_tau is computed by ridge OLS on
    each training day. Reliability uses positive-only factor IC
    (clip(max(factor_ic, 0), 0, 0.10)) so anti-correlated past days
    contribute zero weight.

    Recommended first run: train_efr_params=False (default). Only the
    DOW backbone is optimised; EFR overlay is purely deterministic.
"""
...
from src.v2.model.episodic_factor_repricing import (
    EFRConfig, EpisodicFactorRepricingMemory,
)
...
@dataclass
class TrainConfig:
    """Top-level hyperparameters for DOW-EFR-epiSTAR training."""
    ...
    output_dir: str = "results/dow_efr_epistar"
    # Locked DOW knobs (kept on by default; tracking the v2.3 FULL config).
    ...
    duration_graph_min_warmup_days: int = 0
    duration_graph_source: str = "deterministic_features"
    # EFR overlay knobs.
    use_efr: bool = True
    max_lambda: float = 0.12
    n_retrieved_days: int = 16
    ridge_eta: float = 0.25
    retrieval_temperature: float = 0.20
    use_positive_factor_ic_only: bool = True
    age_protect_days: float = 252.0
    age_protect_steepness: float = 60.0
    train_efr_params: bool = False
    # Ablation: shuffle factor exposures across tickers within each day.
    shuffle_factor_exposures: bool = False
...
# Per-active-ticker feature column indices from x_raw used by the
# DurationExposureEncoder. Same mapping as locked DOW-epiSTAR.
...
def main(cfg_path: str, fold: int, seed: int, smoke: bool = False) -> None:
    """Train DOW-EFR-epiSTAR on one (fold, seed) pair."""
    ...
    print(f"[DOW-EFR] fold={fold} seed={seed} device={device}")
    ...
    print(f"[DOW-EFR] panel: T={x_raw.shape[0]} N={x_raw.shape[1]} F={x_raw.shape[2]}")
    if x_raw.shape[1] < 50:
    ...
    print(f"[DOW-EFR] tradable_cells={int(tradable.sum())} loss_cells={int(loss_mask.sum())}")

    train_idx, val_idx, test_idx = fold_indices(fold, dates)
    print(f"[DOW-EFR] fold {fold}: train={len(train_idx)} val={len(val_idx)} test={len(test_idx)}")
    ...
    cs_disp_dk = day_keys[:, EPISODE_KEY_COLS.index("cs_dispersion")]
    ...
    print(f"[DOW-EFR] IPO memory entries: {len(flat_keys)}")
    ...
    print(f"[DOW-EFR] macro features: {len(macro_cols)} dims")
    ...
    bt_train = betas_tensor[train_idx]; train_mask = tradable[train_idx]
    ...
    dur_norm = np.linalg.norm(duration_input_full, axis=-1)
    ...
    for t in range(len(dates)):
        ...
        dur_summary[t, 2] = float(np.mean(sorted_dn[-n_dec:]))
        dur_summary[t, 3] = float(np.mean(sorted_dn[:n_dec]))
    ...
    # ----- EFR overlay setup -----
    # 12-d regime keys (cs_struct + 8 macro/sector features).
    ...
    regime_keys_arr, regime_cols = build_regime_keys(
        ...
        avg_pairwise_corr_60d=avg_corr_60d, cs_dispersion=cs_disp_dk,
        xbi_close=xbi_close, macro_arr=macro_arr, macro_cols=macro_cols,
        panel_dates=dates, train_idx=train_idx,
    )
    print(f"[DOW-EFR] regime keys: {regime_keys_arr.shape[1]} cols")

    # Factor exposures: PRIMARY_FACTOR_COLS (14 cross-sectionally
    # standardised cols).
    factor_exposures, factor_cols = build_factor_exposures(
        x_raw, age_feat, betas_tensor, tradable,
        factor_cols=list(PRIMARY_FACTOR_COLS), clip_z=5.0,
    )
    if train_cfg.shuffle_factor_exposures:
        rng4 = np.random.default_rng(seed + 7000)
        for t in range(factor_exposures.shape[0]):
            idx_a = np.where(tradable[t])[0]
            if idx_a.size > 1:
                perm = rng4.permutation(idx_a)
                factor_exposures[t, perm] = factor_exposures[t, idx_a]
        print("[DOW-EFR] factor exposures SHUFFLED (sanity ablation)")
    print(f"[DOW-EFR] factor exposures: {factor_exposures.shape[-1]} cols")
    ...
    # EFR memory (no learnable params; theta retrieved deterministically).
    efr_obj_cfg = EFRConfig(
        n_retrieved_days=train_cfg.n_retrieved_days,
        ridge_eta=train_cfg.ridge_eta,
        retrieval_temperature=train_cfg.retrieval_temperature,
        horizon_days=train_cfg.horizon_days, embargo_days=5,
        min_lambda=0.0, max_lambda=train_cfg.max_lambda,
    )
    efr_memory = EpisodicFactorRepricingMemory(
        efr_obj_cfg, regime_dim=regime_keys_arr.shape[1],
        n_factors=factor_exposures.shape[-1],
    ).to(device)
    if train_cfg.use_efr:
        efr_memory.populate(
        ...
        print(f"[DOW-EFR] memory populated with "
              f"{efr_memory.regime_keys.shape[0]} train days "
              f"(out of {len(train_idx)} train days)")
        if efr_memory.regime_keys.shape[0] > 0:
            ic_arr = efr_memory.factor_ics.detach().cpu().numpy()
            print(f"[DOW-EFR] memory factor_ic: "
                  f"mean={float(np.mean(ic_arr)):+.4f} "
                  f"frac>0={float(np.mean(ic_arr > 0)):.2f}")
    ...
    def forward_one_day(t_idx: int) -> dict:
        ...
        use_dgraph = train_cfg.use_duration_graph and dow_cfg.use_duration_graph
        ...
        if use_dgraph:
            with torch.no_grad():
                m_in = torch.from_numpy(macro_arr[t_idx]).float().to(device)
        ...
        score_dow_full = out["y_hat"].float()                          # [N]
        score_dow_active = score_dow_full[active_idx]                  # [A]

        # ----- EFR overlay -----
        # theta + confidence retrieved without gradient (no learnable
        # factor premia). score_efr is computed on the active rows and
        # cs-zscored in float32.
        if train_cfg.use_efr and efr_memory.regime_keys.shape[0] > 0:
            regime_t = torch.from_numpy(regime_keys_arr[t_idx]).float().to(device)
            b_t = torch.from_numpy(
                factor_exposures[t_idx, active_idx_np]
            ).float().to(device)
            with torch.no_grad():
                theta_t, retr_diag = efr_memory.retrieve_theta(
                    regime_key_t=regime_t, query_day_idx=int(t_idx),
                    use_positive_factor_ic_only=train_cfg.use_positive_factor_ic_only,
                )
                confidence = retr_diag["confidence"].float()
            score_efr_active = (b_t @ theta_t.float())                 # [A]
            sf_mean = score_efr_active.mean()
            sf_sd = score_efr_active.std(unbiased=False)
            if float(sf_sd.item()) < 1e-9:
                score_efr_z = torch.zeros_like(score_efr_active)
            else:
                score_efr_z = (score_efr_active - sf_mean) / (sf_sd + 1e-6)

            # Per-ticker age gate.
            age_t_active = torch.from_numpy(
                age_feat[t_idx, active_idx_np, 0]
            ).float().to(device)
            age_gate = torch.sigmoid(
                (age_t_active - train_cfg.age_protect_days)
                / max(train_cfg.age_protect_steepness, 1e-6)
            )

            lambda_efr_active = (
                train_cfg.max_lambda * confidence * age_gate
            )                                                          # [A]

            score_total_active = score_dow_active + lambda_efr_active * score_efr_z

            score_total_full = torch.zeros_like(score_dow_full)
            score_total_full[active_idx] = score_total_active.float()
            score_total_full = score_total_full * active_mask_t.float()

            efr_diag = {
                "lambda_efr_active": lambda_efr_active.detach(),
                "confidence": confidence.detach(),
                "age_gate_mean": age_gate.mean().detach(),
                "score_efr_z": score_efr_z.detach(),
                "mean_reliability": retr_diag["mean_reliability"].detach(),
                "mean_similarity": retr_diag["mean_similarity"].detach(),
                "theta_agreement": retr_diag["theta_agreement"].detach(),
            }
        else:
            score_total_full = score_dow_full
            efr_diag = {
                "lambda_efr_active": torch.zeros(
                    active_idx.numel(), device=device
                ),
                "confidence": torch.zeros((), device=device),
                "age_gate_mean": torch.zeros((), device=device),
                "score_efr_z": torch.zeros(active_idx.numel(), device=device),
                "mean_reliability": torch.zeros((), device=device),
                "mean_similarity": torch.zeros((), device=device),
                "theta_agreement": torch.zeros((), device=device),
            }

        out["y_hat"] = score_total_full
        out["score_dow_full"] = score_dow_full.detach()
    ...
    def evaluate(idx, eval_mask_arr):
        ...
        score_dow_all = np.zeros((T, N), dtype=np.float32)
        ...
        score_efr_all = np.zeros((T, N), dtype=np.float32)
        lambda_efr_per_day = np.zeros(T, dtype=np.float32)
        confidence_per_day = np.zeros(T, dtype=np.float32)
        age_gate_per_day = np.zeros(T, dtype=np.float32)
        lambda_log: list[float] = []
        alpha_rate_log: list[float] = []
        lambda_efr_log: list[float] = []
        confidence_log: list[float] = []
        rel_log: list[float] = []
        sim_log: list[float] = []
        agree_log: list[float] = []
        ...
        for t_idx in idx:
            ...
            sdf = out.get("score_dow_full")
            if sdf is not None:
                score_dow_all[t_idx] = sdf.detach().cpu().numpy()
            ...
            # EFR diagnostics.
            ai_np = out["active_idx"].detach().cpu().numpy()
            sef_z = out["score_efr_z"].detach().cpu().numpy()
            score_efr_all[t_idx, ai_np] = sef_z
            lae = out["lambda_efr_active"].detach().cpu().numpy()
            if lae.size > 0:
                lambda_efr_log.append(float(np.mean(lae)))
                lambda_efr_per_day[t_idx] = float(np.mean(lae))
            conf_v = out["confidence"]
            if isinstance(conf_v, torch.Tensor) and conf_v.dim() == 0:
                confidence_log.append(float(conf_v.item()))
                confidence_per_day[t_idx] = float(conf_v.item())
            age_v = out["age_gate_mean"]
            if isinstance(age_v, torch.Tensor) and age_v.dim() == 0:
                age_gate_per_day[t_idx] = float(age_v.item())
            mr = out.get("mean_reliability")
            ms = out.get("mean_similarity")
            mg = out.get("theta_agreement")
            if isinstance(mr, torch.Tensor) and mr.dim() == 0:
                rel_log.append(float(mr.item()))
            if isinstance(ms, torch.Tensor) and ms.dim() == 0:
                sim_log.append(float(ms.item()))
            if isinstance(mg, torch.Tensor) and mg.dim() == 0:
                agree_log.append(float(mg.item()))
        ...
        ic_dow, _ = per_day_ic(score_dow_all, y, emask, rank=False)
        ...
        return {"ic": ic, "rank_ic": rank_ic, "ndcg10": ndcg10, "ndcg50": ndcg50,
                ...
                "lambda_efr_mean": float(np.mean(lambda_efr_log)) if lambda_efr_log else 0.0,
                "lambda_efr_std": float(np.std(lambda_efr_log)) if lambda_efr_log else 0.0,
                "confidence_mean": float(np.mean(confidence_log)) if confidence_log else 0.0,
                "mean_reliability": float(np.mean(rel_log)) if rel_log else 0.0,
                "mean_similarity": float(np.mean(sim_log)) if sim_log else 0.0,
                "theta_agreement": float(np.mean(agree_log)) if agree_log else 0.0,
                "y_hat_all": y_hat_all,
                "score_dow_all": score_dow_all,
                "score_idio_all": score_idio_all,
                "score_duration_all": score_dur_all,
                "score_rate_all": score_rate_all,
                "score_efr_all": score_efr_all,
                "lambda_efr_per_day": lambda_efr_per_day,
                "confidence_per_day": confidence_per_day,
                "age_gate_per_day": age_gate_per_day,
                "eval_mask": emask}
    ...
    for epoch in range(train_cfg.epochs):
        ...
        for t_idx in perm:
            ...
            lam_act = out.get("lambda_efr_active")
            if lam_act is not None and lam_act.numel() > 0:
                l_efr_size = (lam_act ** 2).mean()
            else:
                l_efr_size = torch.zeros((), device=out["y_hat"].device)
            loss = l_mse + 1e-4 * l_efr_size
            optim.zero_grad()
        ...
        print(f"[DOW-EFR] epoch {epoch}: loss={train_loss:.4f} "
              f"val_ic={val_metrics['ic']:.4f} "
              f"val_ic_dow={val_metrics['ic_dow']:.4f} "
              f"lam_efr={val_metrics['lambda_efr_mean']:.3f} "
              f"conf={val_metrics['confidence_mean']:.3f}")
        if val_metrics["ic"] > best_val_ic + 1e-5:
            ...
            if patience >= train_cfg.early_stop_patience:
                print(f"[DOW-EFR] early stop at epoch {epoch}")
                break
    ...
    np.savez_compressed(
        ...
        score_dow=test_metrics["score_dow_all"],
        ...
        score_efr=test_metrics["score_efr_all"],
        lambda_efr_per_day=test_metrics["lambda_efr_per_day"],
        confidence_per_day=test_metrics["confidence_per_day"],
        age_gate_per_day=test_metrics["age_gate_per_day"],
        y_true=y,
        ...
        regime_keys=regime_keys_arr.astype(np.float32),
    )
    if best_state is not None:
    ...
    for tm in (test_metrics, val_metrics_final):
        for k in ("y_hat_all", "score_dow_all", "score_idio_all",
                  "score_duration_all", "score_rate_all", "score_efr_all",
                  "lambda_efr_per_day", "confidence_per_day", "age_gate_per_day",
                  "eval_mask"):
            tm.pop(k, None)

    payload = {
        "fold": fold, "seed": seed, "model": "DOW-EFR-epiSTAR",
        ...
        "n_efr_memory_entries": int(efr_memory.regime_keys.shape[0]),
        "ic": test_metrics["ic"], "rank_ic": test_metrics["rank_ic"],
        "ic_dow": test_metrics["ic_dow"],
        ...
        "test_mean_reliability": test_metrics["mean_reliability"],
        "test_mean_similarity": test_metrics["mean_similarity"],
        "test_theta_agreement": test_metrics["theta_agreement"],
        "val_ic": val_metrics_final["ic"], "val_rank_ic": val_metrics_final["rank_ic"],
        "val_ic_dow": val_metrics_final["ic_dow"],
        ...
        "config": {
            ...
            "use_efr": bool(train_cfg.use_efr),
        },
    }
    with open(out_path, "w") as f:
        json.dump(payload, f, indent=2, default=str)
    print(f"[DOW-EFR] wrote {out_path}")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--config", type=str, default="configs/dow_efr_epistar.yaml")
