"""epiSTAR-SBP v1 trainer (revised per the SBP revision spec).

Adds three data-foundation items on top of the original SBP v1
architecture:

    1. 21-day IPO blackout in the active mask
       (``src.v2.data.active_mask``).
    2. Shumway-Warther delisting imputation for performance-related
       Nasdaq delistings (-0.55 terminal return spread over the last 5
       trading days). For the current 244-ticker panel this is a no-op
       because the panel has zero detected delistings; the
       infrastructure is in place for an expanded universe.
    3. Four free macro features (DGS10, BAA10Y as proxy for
       BAMLH0A0HYM2, IBB beta 60d, XBI beta 60d) appended to the 22-feature
       panel for a total of 26 features.

The model architecture (cohort-augmented retrieval + dual-pool +
two-head xattn + per-ticker gate + IRF + V-REx + alpha-prior) is
unchanged from train_epistar_sbp.py.
"""
...
from src.v2.data.active_mask import IPOBlackoutConfig, build_blackout_active_mask
...
from src.v2.data.delisting_v1 import DelistingConfig, apply_delisting_imputation
from src.v2.data.episode_keys import (
    EpisodeKeyConfig, EPISODE_KEY_COLS, build_episode_keys,
)
from src.v2.data.macro_features import (
    MacroFeaturesConfig, build_macro_features, standardize_macro,
)
from src.v2.data.minimal_masks import (
    MinimalMaskConfig, build_minimal_masks,
)
...
@dataclass
class TrainConfig:
    """Top-level hyperparameters for epiSTAR-SBP v1 (revised)."""
    ...
    output_dir: str = "results/epistar_sbp_v1"
    # SBP loss params
    ...
    use_delisting_imputation: bool = True
    use_macro_features: bool = True
    # Control mode (epiSTAR-full reproduction without retrieval/loss fixes).
...
def build_episode_values(keys, panel_x, panel_mask):
    """Per-day episode value: cohort-augmented key + 9 summary stats."""
...
def main(cfg_path: str, fold: int, seed: int, smoke: bool = False) -> None:
    """Train epiSTAR-SBP v1 on one (fold, seed) pair."""
    ...
    print(f"[epiSTAR-SBP-v1] fold={fold} seed={seed} device={device} "
          f"control_mode={train_cfg.control_mode}")
    ...
    x_base = tens["x"]; y = tens["y"]; mask_panel = tens["mask"]
    print(f"[epiSTAR-SBP-v1] panel: T={x_base.shape[0]} N={x_base.shape[1]} "
          f"F={x_base.shape[2]}")
    if x_base.shape[1] < 50:
        raise RuntimeError(f"Panel has only {x_base.shape[1]} active tickers; aborting.")

    # Minimal masks (Addition 1 input).
    mm = build_minimal_masks(
        dates, tickers, MinimalMaskConfig(horizon_days=train_cfg.horizon_days)
    )
    tradable = mm["tradable_mask"]
    label_mask_raw = mm["label_mask"]

    # Addition 1: 21-day IPO blackout AND'd into the panel mask.
    blackout_cfg = IPOBlackoutConfig(
        use_ipo_blackout=train_cfg.use_ipo_blackout,
        ipo_blackout_days=train_cfg.ipo_blackout_days,
    )
    mask, blackout_diag = build_blackout_active_mask(mask_panel, tradable, blackout_cfg)
    print(f"[epiSTAR-SBP-v1] blackout: {blackout_diag}")

    # Addition 2: Shumway-Warther delisting imputation. No-op when log
    # is empty (current 244-ticker panel has no detected delistings).
    delisting_diag = {"applied_cells": 0, "tickers_imputed": 0, "by_reason": {}}
    if train_cfg.use_delisting_imputation:
        y, lm_after, delisting_diag = apply_delisting_imputation(
            y, label_mask_raw, dates, tickers, DelistingConfig(),
        )
        print(f"[epiSTAR-SBP-v1] delisting: {delisting_diag}")

    # Addition 3: macro features (4 dims). Ensure parquet exists.
    if train_cfg.use_macro_features:
        macro_path = Path("data/macro_features_v1.parquet")
        if not macro_path.exists():
            print("[epiSTAR-SBP-v1] macro parquet missing; building...")
            build_macro_features(MacroFeaturesConfig())

    # Folds.
    train_idx, val_idx, test_idx = fold_indices(fold, dates)
    print(f"[epiSTAR-SBP-v1] fold {fold}: train={len(train_idx)} val={len(val_idx)} "
          f"test={len(test_idx)}")

    # Concatenate macro features onto the 22-feature panel -> 26 features.
    if train_cfg.use_macro_features:
        macro_long = pd.read_parquet("data/macro_features_v1.parquet")
        macro = standardize_macro(dates, train_idx, macro_long, tickers)
        macro_tensor = np.stack(
            [macro["xbi_beta_60d"], macro["ibb_beta_60d"],
             macro["tenyear_yield_z"], macro["hy_spread_z"]], axis=-1,
        ).astype(np.float32)
        x_raw = np.concatenate([x_base, macro_tensor], axis=-1).astype(np.float32)
        print(f"[epiSTAR-SBP-v1] features: {x_base.shape[2]} base + 4 macro = "
              f"{x_raw.shape[2]} total")
        if backbone_cfg.feature_dim != x_raw.shape[2]:
            print(f"[epiSTAR-SBP-v1] override backbone.feature_dim "
                  f"{backbone_cfg.feature_dim} -> {x_raw.shape[2]}")
            backbone_cfg.feature_dim = x_raw.shape[2]
            sbp_cfg.backbone = backbone_cfg
    else:
        x_raw = x_base.astype(np.float32)

    x = standardize_features(x_raw, mask, train_idx).astype(np.float32)

    # Age features and cohort buckets (computed on tradable mask, not
    # the blackout-protected active mask, so the cohort labels remain
    # well-defined for cells masked out by the 21-day blackout).
    ...
    print(f"[epiSTAR-SBP-v1] cohort buckets per (day, ticker) computed: "
          f"shape={cohort_bkt.shape}")

    # Cohort-augmented 18-dim key.
    ...
    print(f"[epiSTAR-SBP-v1] keys: {keys.shape[1]} dims, values: {values.shape[1]} dims")

    # Dynamic correlation graph.
    dyn_cfg = DynamicGraphConfig(
        window_days=train_cfg.correlation_window, top_k=backbone_cfg.num_neighbors,
    )
    ...
    print(f"[epiSTAR-SBP-v1] dynamic graph: window={dyn_cfg.window_days}d "
    ...
    print(f"[epiSTAR-SBP-v1] train cohort freq: {irf_freq.detach().cpu().numpy().tolist()}")

    # Control mode disables retrieval and loss revisions.
    if train_cfg.control_mode:
        sbp_cfg.use_dual_pool = False
        sbp_cfg.use_two_head_xattn = False
        sbp_cfg.use_per_ticker_gate = False
    ...
    def forward_one_day(t_idx: int) -> dict:
        ...
        mu_input = torch.tensor(
            [regime_scalars[0].item(), regime_scalars[1].item(),
             float(frac_first_126d[t_idx])],
    ...
    def composite_loss(out: dict, lambda_vrex_eff: float) -> tuple[torch.Tensor, dict]:
        ...
        if not train_cfg.control_mode and train_cfg.lambda_irf > 0:
        ...
        if not train_cfg.control_mode and lambda_vrex_eff > 0:
            yh = out["y_hat"][active_idx]; yt = y_true_t[active_idx]
            if yt.numel() >= 2:
        ...
        l_alpha = (alpha_beta_prior(out["alpha_per_ticker"])
                   if (a > 0 and not train_cfg.control_mode)
    ...
    for epoch in range(train_cfg.epochs):
        ...
        for t_idx in perm:
            ...
            if not out:
                continue
            loss, _parts = composite_loss(out, lambda_vrex_eff)
            optim.zero_grad()
        ...
        print(f"[epiSTAR-SBP-v1] epoch {epoch}: loss={train_loss:.4f} "
              f"val_ic={val_metrics['ic']:.4f} alpha={val_metrics['alpha_mean']:.3f} "
              f"mu={val_metrics['mu_mean']:.3f}")
        if val_metrics["ic"] > best_val_ic + 1e-5:
            ...
            if patience >= train_cfg.early_stop_patience:
                print(f"[epiSTAR-SBP-v1] early stop at epoch {epoch}")
                break
    ...
    payload = {
        "fold": fold, "seed": seed,
        "model": "epiSTAR-SBP-v1-control" if train_cfg.control_mode else "epiSTAR-SBP-v1",
        ...
        "blackout": blackout_diag,
        "delisting": delisting_diag,
        ...
        "config": {
            ...
            "model": {"cohort_dim": sbp_cfg.cohort_dim,
    ...
    print(f"[epiSTAR-SBP-v1] wrote {out_path}")


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments."""
    p = argparse.ArgumentParser()
    p.add_argument("--config", type=str, default="configs/epistar_sbp_v1.yaml")
