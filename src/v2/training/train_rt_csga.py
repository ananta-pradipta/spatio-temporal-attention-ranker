"""RT-CSGA trainer: OW-epiSTAR v1 + Regime-Tempered Cross-Sectional Graph Attention.

Per spec Section 5.5 of `docs/specs/rt_csga_spec.md`. Builds on
train_ow_dgraph.py: keeps OW v1 base intact, replaces the duration
graph topology with RT-CSGA's tempered attention head, optionally
adds the auxiliary branch (dropped at inference).

Final per-day score:
    score_total[i] = score_idio[i] (from OW v1's rank head)
                   + score_struct[i] (from RTCSGAHead)
                   + score_aux[i] (training only, ZEROED at inference)
"""
...
from src.v2.data.regime_indicator import (
    RegimeIndicatorConfig, build_regime_indicator_array,
)
...
from src.v2.model.rt_csga import RTCSGAConfig, RTCSGAModel, RTCSGAModelConfig
...
@dataclass
class TrainConfig:
    """Top-level RT-CSGA training hyperparameters."""
    ...
    output_dir: str = "results/rt_csga"
    # Ablation flags (subset of spec Section 8).
    use_regime_temperature: bool = True
    fixed_tau: float = 1.0
    use_auxiliary_branch: bool = True
    aux_gate_init: float = -5.0
    use_random_edges: bool = False
    shuffle_regime_for_test: bool = False
...
def main(cfg_path: str, fold: int, seed: int, smoke: bool = False) -> None:
    """Train RT-CSGA on one (fold, seed) pair."""
    ...
    rt_cfg_d = raw.get("model", {}).get("rt_csga", {})
    rt_csga_cfg = RTCSGAConfig(
        d=backbone_cfg.hidden_dim,
        d_attn=rt_cfg_d.get("d_attn", 64),
        f_single=len(DURATION_GRAPH_FEATURE_COLS),
        f_pair=rt_cfg_d.get("f_pair", 10),
        use_regime_temperature=train_cfg.use_regime_temperature,
        fixed_tau=train_cfg.fixed_tau,
        a_init=rt_cfg_d.get("a_init", 0.0),
        b_init=rt_cfg_d.get("b_init", 1.0),
        use_random_edges=train_cfg.use_random_edges,
        random_edge_seed=seed,
    )
    rt_model_cfg = RTCSGAModelConfig(
        head=rt_csga_cfg,
        use_auxiliary_branch=train_cfg.use_auxiliary_branch,
        aux_hidden_dim=rt_cfg_d.get("aux_hidden_dim", 64),
        aux_gate_init=train_cfg.aux_gate_init,
    )
    if smoke:
        train_cfg.epochs = 2
    ...
    print(f"[RT-CSGA] fold={fold} seed={seed} device={device} "
          f"regime_temp={train_cfg.use_regime_temperature} aux={train_cfg.use_auxiliary_branch} "
          f"random_edges={train_cfg.use_random_edges} shuffle_regime={train_cfg.shuffle_regime_for_test}")
    ...
    print(f"[RT-CSGA] panel: T={x_raw.shape[0]} N={x_raw.shape[1]} F={x_raw.shape[2]}")
    ...
    print(f"[RT-CSGA] fold {fold}: train={len(train_idx)} val={len(val_idx)} test={len(test_idx)}")
    ...
    print(f"[RT-CSGA] IPO memory entries: {len(flat_keys)}")

    # Deterministic 10-d duration feature tensor (same as DOW v2.3 G).
    rolling_betas_path = Path("data/processed/rolling_macro_betas.parquet")
    if not rolling_betas_path.exists():
        print("[RT-CSGA] rolling betas missing; building...")
        build_rolling_betas()
    ...
    print(f"[RT-CSGA] duration features: {duration_feats.shape[-1]} dims")

    # Regime indicator (60-day DGS10 rvol z-score, train-fold scope).
    train_fold_dates = pd.DatetimeIndex(pd.to_datetime([dates[i] for i in train_idx]).normalize())
    regime_arr = build_regime_indicator_array(
        panel_dates=dates, train_fold_dates=train_fold_dates,
        cfg=RegimeIndicatorConfig(),
    )
    if train_cfg.shuffle_regime_for_test:
        rng = np.random.default_rng(seed + 9999)
        # Spec Pitfall 5: shuffle ONLY the test set, not training.
        test_idx_set = set(int(t) for t in test_idx.tolist())
        test_indices = np.array(sorted(test_idx_set), dtype=np.int64)
        regime_test_perm = rng.permutation(regime_arr[test_indices])
        regime_arr = regime_arr.copy()
        regime_arr[test_indices] = regime_test_perm
        print("[RT-CSGA] regime indicator SHUFFLED (test only) -- A4 ablation")
    print(f"[RT-CSGA] regime indicator built; train mean={regime_arr[train_idx].mean():.3f} "
          f"test mean={regime_arr[test_idx].mean():.3f}")

    # Build OW v1 backbone + RT-CSGA model.
    ...
    rt_model = RTCSGAModel(rt_model_cfg).to(device)

    allowed_train = torch.from_numpy(train_idx).long().to(device)
    optim = AdamW(
        list(ow_model.parameters()) + list(rt_model.parameters()),
    ...
    def forward_one_day(t_idx: int) -> dict:
        ...
        # OW v1 keeps its existing correlation graph for spatial attention
        # over patches. RT-CSGA's structural attention is a SEPARATE
        # cross-sectional head added on top of OW v1's score.
        ...
        d_t = torch.from_numpy(duration_feats[t_idx, active_idx_np]).float().to(device)
        r_t = torch.tensor(float(regime_arr[t_idx]), dtype=torch.float32, device=device)
        ...
        with autocast(enabled=use_amp, dtype=torch.float16):
            ...
            score_idio_full = ow_out["y_hat"]              # [N]
            z_final = ow_out["z_final"]                    # [N, D]
            z_active = z_final[active_idx]                 # [A, D]
            score_struct_active, score_aux_active, rt_diag = rt_model(
                z_active, d_t, r_t,
            )

        score_idio_full = score_idio_full.float()
        # Place struct + aux into [N] tensors aligned to active_idx.
        score_struct_full = torch.zeros_like(score_idio_full)
        score_aux_full = torch.zeros_like(score_idio_full)
        score_struct_full[active_idx] = score_struct_active.float()
        score_aux_full[active_idx] = score_aux_active.float()
        score_total = score_idio_full + score_struct_full + score_aux_full
        score_total = score_total * active_mask_t.float()

        return {
            "y_hat": score_total,
            "score_idio": score_idio_full.detach(),
            "score_struct": score_struct_full.detach(),
            "score_aux": score_aux_full.detach(),
            "tau": rt_diag["tau"].detach() if isinstance(rt_diag.get("tau"), torch.Tensor) else torch.tensor(0.0),
            "alpha_entropy": rt_diag["alpha_entropy"].detach(),
    ...
    def evaluate(idx, eval_mask_arr):
        ow_model.eval(); rt_model.eval()
        T = x.shape[0]; N = x.shape[1]
        y_hat_all = np.zeros((T, N), dtype=np.float32)
        score_idio_all = np.zeros((T, N), dtype=np.float32)
        score_struct_all = np.zeros((T, N), dtype=np.float32)
        score_aux_all = np.zeros((T, N), dtype=np.float32)
        tau_log: list[float] = []
        ent_log: list[float] = []
        ...
        for t_idx in idx:
            ...
            score_idio_all[t_idx] = out["score_idio"].cpu().numpy()
            score_struct_all[t_idx] = out["score_struct"].cpu().numpy()
            score_aux_all[t_idx] = out["score_aux"].cpu().numpy()
            tau_log.append(float(out["tau"].item()))
            ent_log.append(float(out["alpha_entropy"].item()))
        ...
        score_aux_max = float(np.max(np.abs(score_aux_all))) if score_aux_all.size > 0 else 0.0
        return {"ic": ic, "rank_ic": rank_ic, "ndcg10": ndcg10, "ndcg50": ndcg50,
                "cohort_ic": coh,
                "tau_mean": float(np.mean(tau_log)) if tau_log else 0.0,
                "tau_std": float(np.std(tau_log)) if tau_log else 0.0,
                "alpha_entropy_mean": float(np.mean(ent_log)) if ent_log else 0.0,
                "score_aux_max_abs": score_aux_max,
                "y_hat_all": y_hat_all,
                "score_idio_all": score_idio_all,
                "score_struct_all": score_struct_all,
                "score_aux_all": score_aux_all,
                "eval_mask": emask}

    step = 0; smoke_step_cap = 80
    for epoch in range(train_cfg.epochs):
        ow_model.train(); rt_model.train()
        ...
        history.append({
            ...
            "val_alpha_entropy_mean": val_metrics["alpha_entropy_mean"],
            "val_score_aux_max_abs": val_metrics["score_aux_max_abs"],
        })
        print(f"[RT-CSGA] epoch {epoch}: train_loss={train_loss:.4f} "
              f"val_ic={val_metrics['ic']:.4f} tau={val_metrics['tau_mean']:.3f}")
        if val_metrics["ic"] > best_val_ic + 1e-5:
            best_val_ic = val_metrics["ic"]
            best_state = {
                "ow": {k: v.detach().clone() for k, v in ow_model.state_dict().items()},
                "rt": {k: v.detach().clone() for k, v in rt_model.state_dict().items()},
            }
            patience = 0
        else:
            patience += 1
            if patience >= train_cfg.early_stop_patience:
                print(f"[RT-CSGA] early stop at epoch {epoch}")
                break

    if best_state is not None:
        ow_model.load_state_dict(best_state["ow"])
        rt_model.load_state_dict(best_state["rt"])
    ...
    np.savez_compressed(
        ...
        score_struct=test_metrics["score_struct_all"],
        score_aux=test_metrics["score_aux_all"],
        y_true=y,
        loss_mask=test_metrics["eval_mask"], tradable_mask=tradable,
        regime_arr=regime_arr.astype(np.float32),
    ...
    for tm in (test_metrics, val_metrics_final):
        for k in ("y_hat_all", "score_idio_all", "score_struct_all",
                  "score_aux_all", "eval_mask"):
            tm.pop(k, None)

    payload = {
        "fold": fold, "seed": seed, "model": "RT-CSGA",
        ...
        "test_tau_mean": test_metrics["tau_mean"],
        "test_tau_std": test_metrics["tau_std"],
        "test_alpha_entropy_mean": test_metrics["alpha_entropy_mean"],
        "test_score_aux_max_abs": test_metrics["score_aux_max_abs"],
        ...
        "config": {"train": asdict(train_cfg),
                   ...
                   "rt_csga": asdict(rt_csga_cfg),
                   "rt_model": asdict(rt_model_cfg),
                   "duration_features": list(DURATION_GRAPH_FEATURE_COLS)},
    }
    with open(out_path, "w") as f:
        json.dump(payload, f, indent=2, default=str)
    print(f"[RT-CSGA] wrote {out_path}")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--config", type=str, default="configs/rt_csga.yaml")
