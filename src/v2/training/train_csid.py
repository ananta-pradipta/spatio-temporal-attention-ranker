...
``docs/specs/csid_implementation_prompt.md``:

    OW-epiSTAR v1 base (preserved unchanged)
    + Section G duration-aware graph (preserved unchanged from DOW headline)
    + CSID layer spliced between STAR's z_active and the
      cross-attention queries / rank head.

Sections E (macro-duration head + lambda gate) and F (rate memory)
are DISABLED -- the CSID layer subsumes their role of regime-conditioned
adjustment via a single embedding-space residualisation.
"""
...
from src.v2.model.csid import CSID
...
@dataclass
class TrainConfig:
    """Top-level CSID v1 training hyperparameters."""
    ...
    output_dir: str = "results/csid_v1"
...
def main(cfg_path: str, fold: int, seed: int, smoke: bool = False) -> None:
    """Train CSID v1 on one (fold, seed) pair."""
    ...
    macro_cfg = MacroStateConfig(
        ...
        gate_state_dim=csid_cfg.get("m_state_dim", 16),
    ...
    print(f"[CSID] fold={fold} seed={seed} device={device}")
    ...
    print(f"[CSID] panel: T={x_raw.shape[0]} N={x_raw.shape[1]} F={x_raw.shape[2]}")
    if x_raw.shape[1] < 50:
    ...
    print(f"[CSID] fold {fold}: train={len(train_idx)} val={len(val_idx)} test={len(test_idx)}")
    ...
    print(f"[CSID] IPO memory entries: {len(flat_keys)}")
    ...
    print(f"[CSID] cs_struct dims: {cs_struct.shape[1]} cols={cs_struct_cols}")
    ...
    # 16-d gate_state output feeds CSID).
    macro_encoder = MacroStateEncoder(macro_cfg).to(device)

    # CSID layer.
    csid = CSID(
        embed_dim=backbone_cfg.hidden_dim,
        cs_struct_dim=cs_struct.shape[1],
        m_state_dim=csid_cfg.get("m_state_dim", 16),
        gate_hidden=csid_cfg.get("gate_hidden", 32),
        gate_init_bias=csid_cfg.get("gate_init_bias", -3.0),
        power_iter_steps=csid_cfg.get("power_iter_steps", 1),
        eps=csid_cfg.get("eps", 1e-6),
    ).to(device)

    allowed_train = torch.from_numpy(train_idx).long().to(device)
    params = list(ow_model.parameters()) + list(macro_encoder.parameters()) + list(csid.parameters())
    ...
    def forward_one_day(t_idx: int) -> dict:
        ...
        with autocast(enabled=use_amp, dtype=torch.float16):
            ...
            def csid_hook(z_active_in):
                z_idio, alpha_csid, v_csid = csid(z_active_in, m_gate_state, cs_t)
                return z_idio, {
                    "alpha_csid": alpha_csid.detach(),
                    "v_csid_norm": v_csid.norm().detach(),
                }
        ...
        out = {
            ...
            "alpha_csid": ow_out.get("hook_alpha_csid", torch.zeros((), device=device)),
            "v_csid_norm": ow_out.get("hook_v_csid_norm", torch.zeros((), device=device)),
        }
        return out

    @torch.no_grad()
    def evaluate(idx, eval_mask_arr):
        ow_model.eval(); macro_encoder.eval(); csid.eval()
        ...
        return {"ic": ic, "rank_ic": rank_ic, "ndcg10": ndcg10, "ndcg50": ndcg50,
                "cohort_ic": coh,
                "alpha_csid_mean": float(np.mean(alpha_log)) if alpha_log else 0.0,
                "alpha_csid_std": float(np.std(alpha_log)) if alpha_log else 0.0,
                "v_csid_norm_mean": float(np.mean(v_norm_log)) if v_norm_log else 0.0,
                "alpha_csid_arr": alpha_arr,
                "y_hat_all": y_hat_all,
                "eval_mask": emask}

    step = 0; smoke_step_cap = 80
    for epoch in range(train_cfg.epochs):
        ow_model.train(); macro_encoder.train(); csid.train()
        ...
        history.append({
            ...
            "val_alpha_csid_mean": val_metrics["alpha_csid_mean"],
            "val_v_csid_norm_mean": val_metrics["v_csid_norm_mean"],
        })
        print(f"[CSID] epoch {epoch}: loss={train_loss:.4f} "
              f"val_ic={val_metrics['ic']:.4f} "
              f"alpha={val_metrics['alpha_csid_mean']:.3f} "
              f"v_norm={val_metrics['v_csid_norm_mean']:.3f}")
    ...
    np.savez_compressed(
        ...
        alpha_csid=test_metrics["alpha_csid_arr"],
        cs_struct=cs_struct,
    )
    if best_state is not None:
    ...
    for tm in (test_metrics, val_metrics_final):
        for k in ("y_hat_all", "eval_mask", "alpha_csid_arr"):
            tm.pop(k, None)
    payload = {
        "fold": fold, "seed": seed, "model": "CSID-v1",
        ...
        "test_alpha_csid_mean": test_metrics["alpha_csid_mean"],
        "test_alpha_csid_std": test_metrics["alpha_csid_std"],
        "test_v_csid_norm_mean": test_metrics["v_csid_norm_mean"],
    ...
    print(f"[CSID] wrote {out_path}")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--config", type=str, default="configs/csid_v1.yaml")
