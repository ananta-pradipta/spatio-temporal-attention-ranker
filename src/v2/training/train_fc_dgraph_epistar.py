"""FC-DGraph-epiSTAR trainer (Factor-Calibrated Duration-Graph epiSTAR).

Per spec `fc_dgraph_epistar_implementation_prompt.md`. Strips the
DOW-epiSTAR additive macro-duration head AND rate memory; replaces
them with a low-rank cross-sectional factor calibrator. The duration
graph is kept and now blended with the correlation graph via a
deterministic regime schedule (no learned graph gate).

Final per-day score:
    score_total[t, i] = z(score_ow[t, i])
                      + lambda_factor[t] * z(score_factor[t, i])

where score_ow comes from OW-epiSTAR v1's rank head on the regime-
blended-graph topology, and score_factor = exposures @ theta_t with
theta_t a soft mixture of n_regimes learnt factor templates.

Stages 0-4 from spec Part H map to config flags:
    Stage 1 (DGraph only): use_factor_calibrator=False, use_duration_graph=True
    Stage 2 (FC only): use_factor_calibrator=True, use_duration_graph=False
    Stage 3 (FC-DGraph full): both True (default headline)
"""
...
from src.v2.data.macro_duration_features import (
    MACRO_FEATURE_COLS_FULL, MacroDurationConfig,
...
from src.v2.graph.duration_dynamic_edges import DURATION_GRAPH_FEATURE_COLS
from src.v2.graph.regime_graph_mixer import (
    RegimeMixerConfig, compute_stress_scalar, merge_topk, stress_to_weights,
)
...
from src.v2.model.factor_calibrator import FactorCalibrator, FactorCalibratorConfig, masked_zscore
...
@dataclass
class TrainConfig:
    """Top-level FC-DGraph-epiSTAR training hyperparameters."""
    ...
    output_dir: str = "results/fc_dgraph_epistar"
    # FC-DGraph specific
    use_duration_graph: bool = True
    use_factor_calibrator: bool = True
    use_learned_lambda: bool = True
    use_regime_schedule: bool = True       # if False, fixed w_corr=0.7 / w_duration=0.3
    fixed_w_corr: float = 0.7
    fixed_w_duration: float = 0.3
    factor_l2_coef: float = 1e-4
    # Ablation flags
    shuffle_factors: bool = False
    shuffle_regime: bool = False
    drop_volatility_factors: bool = False
    drop_cash_factors: bool = False
    drop_social_factors: bool = False
...
REGIME_FEATURE_COLS = [
    ...
    "xbi_ret_20d", "xbi_rv_20d",
]


def main(cfg_path: str, fold: int, seed: int, smoke: bool = False) -> None:
    """Train FC-DGraph-epiSTAR on one (fold, seed) pair."""
    ...
    print(f"[FC-DGraph] fold={fold} seed={seed} device={device}")
    print(f"[FC-DGraph] use_duration_graph={train_cfg.use_duration_graph} "
          f"use_factor_calibrator={train_cfg.use_factor_calibrator} "
          f"use_learned_lambda={train_cfg.use_learned_lambda} "
          f"use_regime_schedule={train_cfg.use_regime_schedule}")
    ...
    print(f"[FC-DGraph] panel: T={x_raw.shape[0]} N={x_raw.shape[1]} F={x_raw.shape[2]}")
    if x_raw.shape[1] < 50:
    ...
    print(f"[FC-DGraph] fold {fold}: train={len(train_idx)} val={len(val_idx)} test={len(test_idx)}")
    ...
    print(f"[FC-DGraph] IPO memory entries: {len(flat_keys)}")

    # Rolling betas + duration features (10-d) for the duration graph.
    rolling_betas_path = Path("data/processed/rolling_macro_betas.parquet")
    if not rolling_betas_path.exists():
        print("[FC-DGraph] rolling betas missing; building...")
        build_rolling_betas()
    ...
    duration_feats = standardize_features(duration_feats_raw, tradable, train_idx).astype(np.float32)
    print(f"[FC-DGraph] duration-graph features: {duration_feats.shape[-1]} dims")

    # Macro panel for regime features and stress scalar.
    macro_path = Path("data/processed/macro_duration_features.parquet")
    if not macro_path.exists():
        print("[FC-DGraph] macro parquet missing; building...")
        build_macro_duration_features()
    macro = pd.read_parquet(macro_path)
    macro_arr, macro_cols, _ = standardize_macro_duration(macro, dates, train_idx)

    # Train-fold standardised cross-sectional scalars.
    avg_corr_train = avg_corr_60d[train_idx]; cs_disp_train = cs_disp[train_idx]
    avg_corr_z = ((avg_corr_60d - avg_corr_train.mean()) /
    ...
    stress = compute_stress_scalar(macro_arr, macro_cols, avg_corr_z, cs_disp_z)
    if train_cfg.use_regime_schedule:
        w_corr_arr, w_dur_arr = stress_to_weights(stress, RegimeMixerConfig())
    else:
        w_corr_arr = np.full_like(stress, train_cfg.fixed_w_corr)
        w_dur_arr = np.full_like(stress, train_cfg.fixed_w_duration)
    print(f"[FC-DGraph] stress range: [{stress.min():.2f}, {stress.max():.2f}], "
          f"w_dur range: [{w_dur_arr.min():.3f}, {w_dur_arr.max():.3f}]")

    # Factor exposures (14-d, cross-sectionally standardised per day).
    factor_cols = list(PRIMARY_FACTOR_COLS)
    if train_cfg.drop_volatility_factors:
        factor_cols = [c for c in factor_cols
                       if c not in ("realized_vol_20d", "realized_vol_60d")]
    if train_cfg.drop_cash_factors:
        factor_cols = [c for c in factor_cols
                       if c not in ("cash_runway_q", "cash_to_mc", "rd_intensity",
                                    "rolling_rate_beta_60d", "rolling_credit_beta_60d")]
    if train_cfg.drop_social_factors:
        factor_cols = [c for c in factor_cols
                       if c not in ("st_volume_24h", "st_bullish_ratio", "st_labeled_ratio")]
    factor_exposures, factor_cols = build_factor_exposures(
        x_raw, age_feat, betas_tensor, tradable, factor_cols=factor_cols,
    )
    if train_cfg.shuffle_factors:
        rng = np.random.default_rng(seed + 7777)
        for t in range(factor_exposures.shape[0]):
            idx_a = np.where(tradable[t])[0]
            if idx_a.size > 1:
                perm = rng.permutation(idx_a)
                factor_exposures[t, perm] = factor_exposures[t, idx_a]
        print("[FC-DGraph] factor exposures SHUFFLED (sanity ablation)")
    print(f"[FC-DGraph] factor exposures: {factor_exposures.shape[-1]} dims, "
          f"cols={factor_cols}")

    # Regime feature vector per day (8-d): delta_10y_20d, delta_hy_spread_20d,
    # xbi_ret_20d, xbi_rv_20d, avg_pairwise_corr_60d, cross_sectional_dispersion,
    # vix_z, vxn_z. All train-fold standardised.
    regime_feat = np.zeros((len(dates), 8), dtype=np.float32)
    for j, col in enumerate(["delta_10y_20d", "delta_hy_spread_20d",
                              "xbi_ret_20d", "xbi_rv_20d"]):
        if col in macro_cols:
            regime_feat[:, j] = macro_arr[:, macro_cols.index(col)]
    regime_feat[:, 4] = avg_corr_z
    regime_feat[:, 5] = cs_disp_z
    if "vix" in macro_cols:
        regime_feat[:, 6] = macro_arr[:, macro_cols.index("vix")]
    if "vxn" in macro_cols:
        regime_feat[:, 7] = macro_arr[:, macro_cols.index("vxn")]
    if train_cfg.shuffle_regime:
        rng2 = np.random.default_rng(seed + 8888)
        perm = rng2.permutation(regime_feat.shape[0])
        regime_feat = regime_feat[perm]
        print("[FC-DGraph] regime features SHUFFLED (sanity ablation)")

    # Build OW backbone + FactorCalibrator.
    ...
    fc_cfg = FactorCalibratorConfig(
        n_factors=len(factor_cols), regime_dim=regime_feat.shape[1],
        n_regimes=raw.get("model", {}).get("n_regimes", 4),
        router_hidden_dim=raw.get("model", {}).get("router_hidden_dim", 32),
        lambda_init_raw=raw.get("model", {}).get("lambda_init_raw", -2.5),
        use_learned_lambda=train_cfg.use_learned_lambda,
        deterministic_lambda_floor=raw.get("model", {}).get("deterministic_lambda_floor", 0.05),
        deterministic_lambda_scale=raw.get("model", {}).get("deterministic_lambda_scale", 0.25),
    )
    fc_model = FactorCalibrator(fc_cfg).to(device) if train_cfg.use_factor_calibrator else None

    allowed_train = torch.from_numpy(train_idx).long().to(device)
    params = list(ow_model.parameters())
    if fc_model is not None:
        params += list(fc_model.parameters())
    ...
    def _duration_similarity_full(t_idx: int) -> np.ndarray:
        """Cosine similarity over duration_feats[t, active, :] expanded to [N, N]."""
        n = len(tickers)
        out = np.zeros((n, n), dtype=np.float32)
        active_idx = np.where(tradable[t_idx])[0]
        if active_idx.size < 2:
            return out
        d = duration_feats[t_idx, active_idx]
        norm = np.linalg.norm(d, axis=-1, keepdims=True).clip(min=1e-8)
        d_n = d / norm
        sim = d_n @ d_n.T
        out[np.ix_(active_idx, active_idx)] = sim
    ...
    def forward_one_day(t_idx: int) -> dict:
        ...
        if train_cfg.use_duration_graph:
            a_corr = _shrunk_corr_full(int(t_idx), cfg_use)
            a_dur = _duration_similarity_full(int(t_idx))
            w_c = float(w_corr_arr[t_idx])
            w_d = float(w_dur_arr[t_idx])
            top = merge_topk(a_corr, a_dur, w_c, w_d, backbone_cfg.num_neighbors, tradable[t_idx])
        ...
        with autocast(enabled=use_amp, dtype=torch.float16):
            ...
            score_ow = ow_out["y_hat"].float()

            if fc_model is not None:
                exp_t = torch.from_numpy(factor_exposures[t_idx]).float().to(device)
                reg_t = torch.from_numpy(regime_feat[t_idx]).float().to(device)
                if not fc_cfg.use_learned_lambda:
                    det_lam = torch.tensor(
                        fc_cfg.deterministic_lambda_floor
                        + fc_cfg.deterministic_lambda_scale * float(w_d),
                        dtype=torch.float32, device=device,
                    )
                    score_total, fc_diag = fc_model(
                        score_ow, exp_t, reg_t, active_mask_t.bool(),
                        deterministic_lambda=det_lam,
                    )
                else:
                    score_total, fc_diag = fc_model(
                        score_ow, exp_t, reg_t, active_mask_t.bool(),
                ...
                score_total = masked_zscore(score_ow, active_mask_t.bool()) * active_mask_t.float()
                fc_diag = {
                    "score_ow_z": score_total.detach(),
                    "score_factor_z": torch.zeros_like(score_total),
                    "lambda_factor": torch.zeros((), device=device),
                    "theta_t": torch.zeros(len(factor_cols), device=device),
                    "regime_probs": torch.zeros(fc_cfg.n_regimes, device=device),
                }

        return {
            "y_hat": score_total.float(),
            "score_ow_z": fc_diag["score_ow_z"],
            "score_factor_z": fc_diag["score_factor_z"],
            "lambda_factor": fc_diag["lambda_factor"],
            "theta_t": fc_diag["theta_t"],
            "w_corr": float(w_c), "w_duration": float(w_d),
            "active_mask": active_mask_t,
            "t_idx": t_idx, "active_idx": active_idx,
        }
    ...
    def evaluate(idx, eval_mask_arr):
        ...
        score_ow_all = np.zeros((T, N), dtype=np.float32)
        score_factor_all = np.zeros((T, N), dtype=np.float32)
        lambda_log: list[float] = []
        w_dur_log: list[float] = []
        emask = np.zeros((T, N), dtype=bool)
        for t_idx in idx:
            out = forward_one_day(int(t_idx))
            if not out: continue
            y_hat_all[t_idx] = out["y_hat"].detach().cpu().numpy()
            score_ow_all[t_idx] = out["score_ow_z"].detach().cpu().numpy()
            score_factor_all[t_idx] = out["score_factor_z"].detach().cpu().numpy()
            lf = out["lambda_factor"]
            if isinstance(lf, torch.Tensor) and lf.dim() == 0:
                lambda_log.append(float(lf.item()))
            w_dur_log.append(out["w_duration"])
        ...
        # IC of components alone
        ic_ow, _ = per_day_ic(score_ow_all, y, emask, rank=False)
        ic_factor, _ = per_day_ic(score_factor_all, y, emask, rank=False)
        return {"ic": ic, "rank_ic": rank_ic, "ndcg10": ndcg10, "ndcg50": ndcg50,
                "cohort_ic": coh,
                "lambda_factor_mean": float(np.mean(lambda_log)) if lambda_log else 0.0,
                "w_duration_mean": float(np.mean(w_dur_log)) if w_dur_log else 0.0,
                "ic_score_ow": ic_ow, "ic_score_factor": ic_factor,
                "y_hat_all": y_hat_all,
                "score_ow_all": score_ow_all,
                "score_factor_all": score_factor_all,
                "eval_mask": emask}
    ...
    for epoch in range(train_cfg.epochs):
        ...
        for t_idx in perm:
            ...
            l_rank = cs_mse_loss(out["y_hat"], y_true_t, loss_mask_t)
            if fc_model is not None and train_cfg.factor_l2_coef > 0:
                l_l2 = (fc_model.theta ** 2).mean()
                loss = l_rank + train_cfg.factor_l2_coef * l_l2
                epoch_l2.append(float(l_l2.item()))
            else:
                loss = l_rank
            optim.zero_grad()
        ...
        history.append({
            ...
            "train_l2": float(np.mean(epoch_l2)) if epoch_l2 else 0.0,
            "val_ic": val_metrics["ic"], "val_rank_ic": val_metrics["rank_ic"],
            "val_lambda_factor_mean": val_metrics["lambda_factor_mean"],
            "val_w_duration_mean": val_metrics["w_duration_mean"],
            "val_ic_score_ow": val_metrics["ic_score_ow"],
            "val_ic_score_factor": val_metrics["ic_score_factor"],
        })
        print(f"[FC-DGraph] epoch {epoch}: loss={train_loss:.4f} "
              f"val_ic={val_metrics['ic']:.4f} "
              f"ow_ic={val_metrics['ic_score_ow']:.4f} "
              f"factor_ic={val_metrics['ic_score_factor']:.4f} "
              f"lambda={val_metrics['lambda_factor_mean']:.3f} "
              f"w_dur={val_metrics['w_duration_mean']:.3f}")
        if val_metrics["ic"] > best_val_ic + 1e-5:
            ...
            if fc_model is not None:
                best_state["fc"] = {k: v.detach().clone() for k, v in fc_model.state_dict().items()}
            patience = 0
        else:
            patience += 1
            if patience >= train_cfg.early_stop_patience:
                print(f"[FC-DGraph] early stop at epoch {epoch}")
                break

    if best_state is not None:
        ow_model.load_state_dict(best_state["ow"])
        if fc_model is not None and "fc" in best_state:
            fc_model.load_state_dict(best_state["fc"])
    ...
    np.savez_compressed(
        ...
        score_ow=test_metrics["score_ow_all"],
        score_factor=test_metrics["score_factor_all"],
        y_true=y,
        ...
        w_duration_arr=w_dur_arr.astype(np.float32),
        stress_arr=stress.astype(np.float32),
    )
    if best_state is not None:
    ...
    for tm in (test_metrics, val_metrics_final):
        for k in ("y_hat_all", "score_ow_all", "score_factor_all", "eval_mask"):
            tm.pop(k, None)

    payload = {
        "fold": fold, "seed": seed, "model": "FC-DGraph-epiSTAR",
        ...
        "test_lambda_factor_mean": test_metrics["lambda_factor_mean"],
        "test_w_duration_mean": test_metrics["w_duration_mean"],
        "test_ic_score_ow": test_metrics["ic_score_ow"],
        "test_ic_score_factor": test_metrics["ic_score_factor"],
        ...
        "config": {"train": asdict(train_cfg),
                   ...
                   "factor_calibrator": asdict(fc_cfg),
                   "factor_cols": factor_cols,
                   "regime_feature_dim": regime_feat.shape[1]},
    }
    with open(out_path, "w") as f:
        json.dump(payload, f, indent=2, default=str)
    print(f"[FC-DGraph] wrote {out_path}")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--config", type=str, default="configs/fc_dgraph_epistar_full.yaml")
